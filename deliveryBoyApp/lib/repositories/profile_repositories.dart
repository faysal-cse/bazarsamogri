...
class ProfileRepository {
  ...
  Future<ProfileUpdateResponse> getProfileUpdateResponse(
    ...
    var postBody = jsonEncode({"id":"${user_id.$}","name": name, "password": password});

    final response = await ApiRequest.post(url: ("${AppConfig.BASE_URL}/profile/update"),
        headers: {"Content-Type": "application/json", "Authorization": "Bearer ${access_token.$}"},body: postBody );
  ...
  Future<ProfileImageUpdateResponse> getProfileImageUpdateResponse(
    ...
    var postBody = jsonEncode({"id":"${user_id.$}","image": image, "filename": filename});
