...
class _AssignedDeliveryState extends State<AssignedDelivery> {
  ...
  final List<Sortable> _datewiseSortList = Sortable.getDatewiseSortList();
  final List<Sortable> _paymentTypeSortList = Sortable.getPaymentTypeSortList();
  ...
  List<DropdownMenuItem<Sortable>> buildDropdownItems(List paymentStatusList) {
    List<DropdownMenuItem<Sortable>> items = [];
    for (Sortable item in paymentStatusList as Iterable<Sortable>) {
      items.add(
        DropdownMenuItem(
          value: item,
  ...
  onConfirmMarkPicked(orderId) async {
    // print(order_id);
    ...
    return Padding(
      ...
      child: Row(
        ...
        children: [
          ...
          Container(
            child: deliveryPoints.isNotEmpty
                ? InkWell(
