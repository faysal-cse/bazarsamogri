...
class MultiOrderMap extends StatefulWidget {
  ...
  const MultiOrderMap({super.key, this.orders});
...
class _MultiOrderMapState extends State<MultiOrderMap> {
  ...
  Iterable _createMarker() {
    Iterable markers = Iterable.generate(widget.orders!.length, (index) {
      return Marker(
  ...
  onPressCenterMap() async {
    _controller.moveCamera(CameraUpdate.newLatLng(
        LatLng(widget.orders!.first.lat!, widget.orders!.first.lang!)));
    setState(() {});
  }
      ...
      child: location_initialized
          ? GoogleMap(
              initialCameraPosition: CameraPosition(
                  target: LatLng(
                      widget.orders!.first.lat!, widget.orders!.first.lang!),
                  zoom: 11.0,
                  tilt: 0.0,
                  bearing: 0),
